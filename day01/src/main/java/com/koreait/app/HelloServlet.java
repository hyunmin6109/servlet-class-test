...
// @WebServleyt 어노테이션은 우리가 지정한 path(경로)로 요청이 들어왔을때
// 이 서블릿이 실행되도록 설절해 준다.
@WebServlet("/hello")
...
//     요청방식이 get | post 둘 중 하나가 들어오게 된다.
//    get 방식으로 요청이 들어오면 doGet()메서드가 실행된다.
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
//		doGet 메서드의 파라미터를 확인해보면 request(요청), response(응답)이 선언되어 있다.
//		자바에서는 요청과 객체로 다루며, 사용자가 보낸 요청에서 필요한 정보를 꺼낼때 request 객체에서 꺼낼수 있다.
